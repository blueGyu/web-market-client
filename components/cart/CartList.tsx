  ...
  return (
    <>
      <main className="flex flex-col justify-between min-h-page px-2.5 md:flex-row md:px-20">
        ...
        <div className="sticky bottom-0 md:top-0 flex flex-col justify-between basis-1/3 min-w-[300px] max-h-page py-2.5 bg-[var(--background-color)]">
          <div className="flex flex-col justify-between w-full border rounded p-2.5 h-[150px] mb-2.5">
            <div>예상 구매 가격</div>
          ...
          <Button onClick={handleShowPay}>상품 결제하기</Button>
        </div>
      </main>
      {isPayed &&
        createPortal(
          <div className="fixed top-0 left-0 z-30 w-screen h-screen flex items-center justify-center bg-slate-400/60">
            <div className="flex flex-col justify-between w-[300px] h-[200px] bg-item-card p-2.5 rounded">
              <button
              ...
              <Button onClick={handleHidePay}>확인</Button>
            </div>
          </div>,
          document.body
        )}
